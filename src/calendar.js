...
function searchText(r){return norm([r.name,r.origin,r.variety||'',...(r.aliases||[])].join(' '))}
function qualityDetails(r){return (r.variety?'<p class="fine"><b>Сорт и название:</b> '+esc(r.variety)+'</p>':'')+(r.selection?'<h3>Как выбрать в магазине</h3><p>'+esc(r.selection)+'</p>':'')+(r.ripening?'<h3>Дозреет ли дома</h3><p>'+esc(r.ripening)+'</p>':'')+(r.qualitySources?.length?'<details><summary>Источники о спелости</summary>'+sourceList(r.qualitySources)+'</details>':'')}
...
function details(id,m=month){const r=DB.rows.find(x=>x.id===id),s=r.months[m];$('dialog-content').innerHTML='<div class="dialog-head"><div><p class="fine">'+esc(r.category)+'</p><h2 id="dialog-title">'+esc(r.name)+'</h2><p>'+esc(r.origin)+'</p></div><button class="close" id="close-dialog" aria-label="Закрыть подробности">×</button></div><div class="detail-note">'+pill(s)+' <strong>· '+MONTHS[m]+'</strong><p>'+esc(DB.statuses[s][2])+'</p></div><p>'+esc(r.note)+'</p>'+qualityDetails(r)+'<div class="year-mini">'+r.months.map((s,i)=>'<div>'+SHORT[i]+symbol(s)+'</div>').join('')+'</div><p><b>Надёжность:</b> '+esc(r.confidence)+'.</p><p><b>Поставки:</b> '+esc(r.supply)+'</p><p class="fine">'+esc(r.basis)+'</p><h3>Основания для этой строки</h3>'+(r.sources.length?sourceList(r.sources):'<p class="notice">Надёжного помесячного подтверждения пока нет. </p>');
